...
A _Connection_ links your owned databases and [third-party applications](/docs/dev-docs/configuration/integrations/saas-integrations/saas-config) to Fides, allowing Fides to execute privacy requests against your collections and fields.
...
## Associate a Dataset
Once you have a working Integration, it must be associated to an existing [dataset](/docs/dev-docs/configuration/integrations/database-integrations/datasets). This enables Fides to map and access the contents of your database. 
...
The `system_status` filter surfaces either `database` or [`saas`-type](/docs/dev-docs/configuration/integrations/saas-integrations/saas-config) connectors:
