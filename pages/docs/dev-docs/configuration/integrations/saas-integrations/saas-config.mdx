...
A [SaaS connector](/docs/dev-docs/configuration/integrations/saas-integrations/saas-config) is defined in two parts: the [Dataset](/docs/user-guides/integrations/database-integrations/datasets), and the SaaS configuration. The Dataset describes the data that is available from the connector, and the SaaS config describes how to connect to, and retrieve or update the data in the connector.

When accessing data from APIs, each application (and different endpoints within the same application) can follow different patterns, making their requirements different from [Database connectors](/docs/dev-docs/configuration/integrations/database-integrations). Fides provides a flexible configuration to define different access/update patterns.

## An example SaaS config

This guide will use a SaaS configuration to connect to [Mailchimp](/docs/dev-docs/configuration/integrations/saas-integrations/example-configs/mailchimp). The configuration schema defines:
...
| `type` | Type of SaaS connector. Choose from `hubspot`, `mailchimp`, or the other [available connectors](/docs/dev-docs/configuration/integrations/saas-integrations/example-configs/adobe), or use `custom` for other types. |
...
Fides also supports [OAuth2 authentication](/docs/dev-docs/configuration/integrations/saas-integrations/saas-oauth2).
...
Once the base client is defined, use a `test_request` to verify the hostname and credentials. This is in the form of an idempotent request (usually a `read` request). The testing approach is the same for any [connection](/docs/dev-docs/configuration/integrations/database-integrations/datasets).
...
| `param_values.references` | These are the same as `references` in a [Dataset](/docs/user-guides/integrations/database-integrations/datasets). It is used to define the source of the value for the given `param_value`.
...
| `postprocessors` | An optional list of response post-processing strategies. We will ignore this for the example scenarios below but an in depth-explanation can be found under [SaaS Post-Processors](/docs/dev-docs/configuration/integrations/saas-integrations/saas-config).
| `pagination` | An optional strategy used to get the next set of results from APIs with resources spanning multiple pages. Details can be found under [SaaS Pagination](/docs/dev-docs/configuration/integrations/saas-integrations/saas-pagination).
...
and the contents of the body would be masked according to the configured [policy](/docs/dev-docs/configuration/privacy-requests/graphs#how-does-fides-execute-queries).
...
Some endpoints might not have any external dependencies on `identity` or Dataset `reference` values. The way the Fides [graph traversal](/docs/dev-docs/configuration/privacy-requests/graphs#graphs-and-traversals) interprets this is as an unreachable collection. At this time, the way to mark this as reachable is to include a `param_value` with an identity or a reference.
