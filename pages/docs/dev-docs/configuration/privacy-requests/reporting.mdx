...
Due to the way Fides stores identity data, this data will expire automatically according to the `FIDES__REDIS__DEFAULT_TTL_SECONDS` variable in your Fides configuration. Read how to set [Fides configuration variables here](/docs/dev-docs/configuration/configuration).
...
logs for `my-postgres-db` (when the `order` collection is starting and finishing execution). The `fields_affected` are the fields that were potentially returned or masked based on the [rules](/docs/dev-docs/configuration/privacy-requests/graphs) you've specified on the privacy request policy. 
...
<<<<<<< HEAD:pages/docs/dev-docs/configuration/privacy-requests/reporting.mdx
See the [manual data guides](/docs/user-guides/integrations/database-integrations/datasets#resume-a-paused-access-privacy-request) for more information on resuming a paused access request.
=======
See the [manual data guides](/dev-docs/configuration/integrations/database-integrations/datasets) for more information on resuming a paused access request.
>>>>>>> main:pages/dev-docs/configuration/privacy-requests/reporting.mdx
...
A confirmation of the masked records count should be uploaded to the `resume_endpoint`. See the [manual data guides](/docs/user-guides/integrations/database-integrations/datasets#resume-a-paused-access-privacy-request) for more information on resuming a paused erasure request.
