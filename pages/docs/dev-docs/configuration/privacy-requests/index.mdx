...
For more information on policies, see the [privacy request policies](/docs/dev-docs/configuration/policies/request-policies) guide.
...
| `policy_key` | Identifies the [privacy request policy](/docs/dev-docs/configuration/privacy-requests/graphs) applied to this request. |
...
2. [Configure Emails](/docs/dev-docs/configuration/privacy-requests/execution#send-email-notifications) that lets Fides send automated emails to your users.
...
For more detailed examples and further privacy request filtering, see [Reporting on Privacy Requests](/docs/dev-docs/configuration/privacy-requests/reporting).
...
* **Generic API interoperability**: Third party services can be authorized by creating additional OAuth clients. Tokens obtained from OAuth clients can be managed and revoked at any time. See [authenticating with OAuth](/docs/dev-docs/configuration/oauth/overview) for more information.