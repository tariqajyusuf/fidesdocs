...
They differ from the more complex [manual connections](/docs/user-guides/integrations/databases) that integrate directly with the graph.
Manual privacy requests gather data *outside* of the graph as a first step, and are similar to [privacy request webhooks](/docs/dev-docs/configuration/privacy-requests/webhooks).
