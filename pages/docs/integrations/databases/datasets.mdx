...
## Configure a dataset
Fides uses a YAML manifest file to represent your datastores, and requires information beyond table names and fields to fully configure a Dataset. Datastores connected in this way will be automatically traversed when Fides executes a privacy request, and will either return or update the requested data according to the associated [execution policy](../../privacy-requests/privacy-request-execution.mdx).

Ensure you have created an [Integration](./overview) for the datastore you would like to map. The Dataset defined by the following process should be [associated to the Connection](./overview#associate-a-dataset).
...
| `fidesops_meta.identity` | Signifies that this field is an identity value that can be used as the root for a traversal. For more information, see [graph traversals](../../privacy-requests/privacy-request-graphs.mdx). |
...
For more information, see [generating resources](../../cli_support/generate-resources).
...
To add a Manual Dataset, first create a [Manual Connection](./overview#examples). The following Manual Dataset can then be [added to](overview#associate-a-dataset) the new ConnectionConfig:
