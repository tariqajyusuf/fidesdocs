...
An _Integration_ links your owned databases and [third-party applications](../saas-integrations/overview) to Fides, allowing Fides to execute privacy requests against your collections and fields.
...
A _[Dataset](./datasets)_ is model of a service that describes the data contained within each field. An _Integration_ stores the secrets to connect to the database. After Fides connects to your database, it dynamically generates queries to fulfil privacy requests by consulting the annotations in the Dataset.
...
The `system_status` filter surfaces either `database` or [`saas`-type](../saas-connectors/overview) connectors:
