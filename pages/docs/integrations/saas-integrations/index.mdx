...
A Fides SaaS (Software as a Service) Integration allows a user to connect to a third-party application (e.g., Mailchimp, Stripe, Hubspot), and execute data access and erasure requests against that application. Fides represents your SaaS integrations as a [Dataset](../databases/datasets), accessed via an [Integration](../databases/database-integrations), and configured by a [SaaS config](./saas-config).
...
For full examples of supported Integrations, see the [example configurations](./example-configs/adobe).
...
### Create an integration of type `saas`
...
The example below creates a [mailchimp](./example_configs/mailchimp) saas integration, and would need the relevant mailchimp `secrets`.

Your `instance_key` will become the identifier for the related [Dataset](../databases/datasets). By default, the saas integration config is enabled with write access.
