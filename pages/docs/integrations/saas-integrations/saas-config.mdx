...
A [SaaS integration](overview) is defined in two parts: the [Dataset](../databases/datasets), and the SaaS configuration. The Dataset describes the data that is available from the integration, and the SaaS config describes how to connect to, and retrieve or update the data in the data store.

When accessing data from APIs, each application (and different endpoints within the same application) can follow different patterns, making their requirements different from [Database connectors](../databases/database-integrations). Fides provides a flexible configuration to define different access/update patterns.

## An example SaaS config

This guide will use a SaaS configuration to connect to [Mailchimp](./example-configs/mailchimp). The configuration schema defines:
...
|----|----|
| `fides_key` | Used to uniquely identify the integration, and to link a SaaS config to a dataset. |
| `name` | A human-readable name for the integration. |
| `type` | Type of SaaS integration. Choose from `hubspot`, `mailchimp`, or the other [available integrations](./example-configs/adobe), or use `custom` for other types. |
...
Fides also supports [OAuth2 authentication](saas-oauth2).
...
Once the base client is defined, use a `test_request` to verify the hostname and credentials. This is in the form of an idempotent request (usually a `read` request). The testing approach is the same for any [Connection](../dsr_support/database_integrationss#test-your-integration).
...
| `param_values.references` | These are the same as `references` in a [Dataset](../databases/datasets). It is used to define the source of the value for the given `param_value`.
...
| `postprocessors` | An optional list of response post-processing strategies. We will ignore this for the example scenarios below but an in depth-explanation can be found under [SaaS Post-Processors](saas-postprocessors).
| `pagination` | An optional strategy used to get the next set of results from APIs with resources spanning multiple pages. Details can be found under [SaaS Pagination](saas-pagination).
...
Fides uses the available Datasets to [generate a graph](../privacy-requests/privacy-request-execution) of all reachable data and the dependencies between Datasets. For SaaS integrations, all the references and identities are stored in the `param_values`, and must merge both the SaaS config and Dataset to provide a complete picture for the graph traversal. Using Mailchimp as an example, the Dataset collection and SaaS config endpoints for `messages` looks like this:
...
Some endpoints might not have any external dependencies on `identity` or Dataset `reference` values. The way the Fides [graph traversal](../privacy-requests/privacy-request-graphs.mdx) interprets this is as an unreachable collection. At this time, the way to mark this as reachable is to include a `param_value` with an identity or a reference.
