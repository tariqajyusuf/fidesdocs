...
A [SaaS connector](saas-connectors) is defined in two parts: the [Dataset](/docs/databases/datasets), and the SaaS configuration. The Dataset describes the data that is available from the connector, and the SaaS config describes how to connect to, and retrieve or update the data in the connector.

When accessing data from APIs, each application (and different endpoints within the same application) can follow different patterns, making their requirements different from [Database connectors](/docs/databases/database-connections). Fides provides a flexible configuration to define different access/update patterns.

## An example SaaS config

This guide will use a SaaS configuration to connect to [Mailchimp](/docs/connectors/example-connectors/mailchimp). The configuration schema defines:
...
| `type` | Type of SaaS connector. Choose from `hubspot`, `mailchimp`, or the other [available connectors](/docs/connectors/example-connectors/adobe), or use `custom` for other types. |
...
Fides also supports [OAuth2 authentication](/docs/connectors/saas-oauth2).
...
Once the base client is defined, use a `test_request` to verify the hostname and credentials. This is in the form of an idempotent request (usually a `read` request). The testing approach is the same for any [connection](/docs/databases/database-connections#test-your-connection).
...
| `param_values.references` | These are the same as `references` in a [Dataset](/docs/databases/datasets). It is used to define the source of the value for the given `param_value`.
...
| `postprocessors` | An optional list of response post-processing strategies. We will ignore this for the example scenarios below but an in depth-explanation can be found under [SaaS Post-Processors](/docs/connectors/saas-postprocessors).
| `pagination` | An optional strategy used to get the next set of results from APIs with resources spanning multiple pages. Details can be found under [SaaS Pagination](/docs/connectors/saas-pagination).
...
and the contents of the body would be masked according to the configured [policy](/docs/privacy-requests/privacy-request-graphs#how-does-fides-execute-queries).
...
Some endpoints might not have any external dependencies on `identity` or Dataset `reference` values. The way the Fides [graph traversal](/docs/privacy-requests/privacy-request-graphs#graphs-and-traversals) interprets this is as an unreachable collection. At this time, the way to mark this as reachable is to include a `param_value` with an identity or a reference.
