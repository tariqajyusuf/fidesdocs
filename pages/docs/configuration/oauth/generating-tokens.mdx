...
* A valid client ID and client secret for a self-hosted instance of Fides. Read the [access configuration guide](#LINK) to set this up now.

## Example: Creating an OAuth Access Token

To create an OAuth access token, make a request to the `POST /api/v1/oauth/token` endpoint with a valid client ID and client secret in the following format:
