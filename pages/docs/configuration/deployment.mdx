...
## Deployment options
Any Docker host can be used to deploy Fides, and this guide outlines three main deployment options: [Helm](#using-helm), [Terraform](#using-terraform), and [Docker](#using-docker). For all options, please refer to our [security best practices](/docs/configuration/security-practices) for recommendations on production infrastructure.
...
Fides uses an application database for persistent storage. Any hosted PostgreSQL database that meets the current [project requirements](/docs/get-started/requirements) is acceptable, as long as it's accessible. 
...
Ensure that Docker is running on your host, and satisfies the [minimum requirements](/docs/get-started/requirements).
