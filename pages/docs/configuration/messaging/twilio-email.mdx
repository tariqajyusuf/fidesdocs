...
The `service_type` should be `twilio_email` and `{{TWILIO_FROM_EMAIL}}` should be the "From Email" you'd like to send messages from.
...
The `twilio_api_key` value should be either your Twilio API Key as created in [this section](#generate-your-twilio-sendgrid-api-key).
...
The `notification_service_type` value should be `twilio_email` and for each of the notifications you wish to send via Twilio, set their value to `true`.
