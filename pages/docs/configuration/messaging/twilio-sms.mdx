...
```
`{{FIDES_URL}}` should be the URL to your Fides server. The Authorization header should be Bearer, where `{{FIDES_ACCESS_TOKEN}}` is your Fides access token and the request Content-Type should be application/json. 

The `service_type` should be `twilio_text`.
...
The `twilio_account_sid` value should be either your Twilio ACCOUNT SID, `twilio_auth_token` should be your auth token and `twilio_messaging_service_sid` should be your messaing service ID as created in [this section](#getting-your-twilio-sms-credentials). 
...
The `notification_service_type` value should be `twilio_text` and for each of the notifications you wish to send via Twilio, set their value to `true`.
