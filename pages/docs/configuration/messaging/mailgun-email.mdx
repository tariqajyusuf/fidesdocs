...
The `service_type` should be `mailgun` and `{{MAILGUN_DOMAIN}}` should be the domain associated with your Mailgun account.
...
The `mailgun_api_key` value should be either your Mailgun Private API Key or Domain Sending Key as described in [this section](#generate-your-mailgun-api-key).
...
The `notification_service_type` value should be `mailgun` and for each of the notifications you wish to send via Mailgun, set their value to `true`.
