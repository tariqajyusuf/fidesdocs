...
Based on your [configuration](../get-started/configuration), Fides can perform a number of other actions: 
...
| **Approve** | If configured, Fides will require a system administrator to [approve](../get-started/configuration) the request before proceeding. |
...
If there is a failure trying to retrieve data on any Collections, the request is retried the number of times [configured](../get-started/configuration) by `task_retry_count` until the request exits with status `error`.  Both the `access` step and errored Collection are cached in temporary storage.
