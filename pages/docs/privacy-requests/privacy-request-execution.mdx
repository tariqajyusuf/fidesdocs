...
Based on your [configuration](/docs/get-started/requirements), Fides can perform a number of other actions: 
...
| **Verify** | If configured, Fides sends an [email](/docs/configuration/messaging/mailgun-email#example-set-messaging-config-variables) to the user to verify their identity before proceeding. |
| **Notify** | If configured, the user will receive an [email](/docs/configuration/messaging/overview) verifying that their request has been received. |
| **Approve** | If configured, Fides will require a system administrator to [approve](/docs/get-started/requirements) the request before proceeding. |
...
Policy pre-execution webhooks let your system take care of prerequisite tasks, or locate additional identities for the data subject. Examples include turning on a specific database in your infrastructure, or locating a phone number for a subject from a table for which you do not want to give Fides direct access. Configuration involves defining endpoint(s) for Fides to call in order. See [Policy Webhooks](/docs/privacy-requests/privacy-request-webhooks) for more details.

Fides sends a request to each pre-execution webhook with a [policy webhooks request format](/docs/privacy-requests/privacy-request-webhooks#webhook-request-format), which your endpoints should be prepared to unpack. If you need more time to carry out an action, your webhook can instruct Fides to `halt`, which will cause execution to exit with a status of `paused`. Request execution can be continued when ready using a token supplied in the original request.
...
Access request automation is performed regardless of whether there are access or erasure Rules defined, as both Rules require this data.  See how to [configure policies, rules, and rule targets](/docs/privacy-requests/privacy-request-graphs) for additional information.

This step visits all Collections and retrieves all Fields that you've defined in your [Datasets](/docs/databases/datasets). Fides builds a graph in accordance with how you've designated your Collections are related, visits each Collection in turn, and gathers all the results together.
...
A Collection isn't visited until Fides has searched for data across all of its upstream Collections. This continues until all Collections have been visited. See [Query Execution](/docs/privacy-requests/privacy-request-graphs) for more information.

![Access Execution](/assets/img/resources/access_execution.png)

If there is a failure trying to retrieve data on any Collections, the request is retried the number of times [configured](/docs/get-started/requirements) by `task_retry_count` until the request exits with status `error`.  Both the `access` step and errored collection are cached in temporary storage.
...
For each configured access Rule, Fides filter the graph results to match targeted Data Categories. See [Datasets](/docs/databases/datasets) for more details.
...
or CSV format to a specified storage location like Amazon S3. See [configuring storage](/docs/configuration/storage-destinations) for more information.


### Erasure request automation
If applicable, (erasure [Rules](/docs/privacy-requests/privacy-request-graphs) are configured on your execution policy), Fides builds a simpler version of the access request graph, and visits each Collection in turn, performing masking requests as necessary.
...
#### Graph execution
Fides visits each Collection sequentially, using a deterministic order set by Dask. For each row of data retrieved in the access request step, Fides attempts to mask the data targeting the fields specified on your execution policy, using the [masking strategies](/docs/privacy-requests/privacy-request-masking-strategies) you've defined. If no rows exist from the access request, or no Fields on that Collection match the targeted Data Categories, no masking occurs. Fides caches a count of the records
...
After automated access and erasures have completed, post-execution webhooks can be used to perform any cleanup steps in your system.  Examples include setting up a webhook to shut down a database, or to delete user data from a source you don't want Fides to access directly.  Post-execution webhooks are more limited than Pre-execution webhooks. They currently cannot pause the graph, and should be configured as a series of API endpoints you would like Fides to call. See [policy webhooks](/docs/privacy-requests/privacy-request-webhooks) for more details on configuration.
