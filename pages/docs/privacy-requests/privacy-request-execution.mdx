...
| **Verify** | If configured, Fides sends an [email](../configuration/messaging/mailgun-email#example-set-messaging-config-variables) to the user to verify their identity before proceeding. |
| **Notify** | If configured, the user will receive an [email](../configuration/messaging/overview) verifying that their request has been received. |
...
If there is a failure trying to retrieve data on any Collections, the request is retried the number of times [configured](../configuration/configuration) by `task_retry_count` until the request exits with status `error`.  Both the `access` step and errored Collection are cached in temporary storage.
