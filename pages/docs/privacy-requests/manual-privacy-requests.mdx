...
They differ from the more complex [manual connections](/docs/databases/datasets#configure-a-dataset) that integrate directly with the graph.
Manual privacy requests gather data *outside* of the graph as a first step, and are similar to [privacy request webhooks](/docs/privacy-requests/privacy-request-webhooks).
