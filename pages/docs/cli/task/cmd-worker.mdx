...
> **Hands-on:** To underestand workers in Fides try the [Product Deployment](#LINK) tutorials.
...
2. Avoiding asyncrhonous race conditions and performance issues.

## Usage
Usage: ```fides workers```

Running this command will start a Celery Worker. As you can see in the example below, the worker is configured to listen for privacy request tasks and execute privacy requests. For more in-depth information on privacy requests read the [Fides Privacy Requests](#LINK) guide.
