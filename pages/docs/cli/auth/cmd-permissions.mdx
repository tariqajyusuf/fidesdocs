...
The ```permissions``` command requires a valid local credentials file with access token. To learn more about creating a credentials file see the [Logging into Fides Server from the CLI](#LINK) guide.
...
This example lists all scopes available for the current user. To successfully run this command you must already be logged in using [```fides user login```](cmd-login) command.
