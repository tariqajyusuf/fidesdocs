...
The ```fides user login``` command can be used to automatically obtain and save an API token for Fides Server.

<Callout emoji="">**Note:** The username and password provided must be an existing, authorized user for Fides. If you have not created at least one user for Fides already, you must complete this step first. For details on creating users, see the [Creating Fides Users](#LINK) guide.</Callout>
...
By default, Fides will obtain an API token and save it in a plain text in a local CLI configuration file called ```.fides_credentials```. When you run ```fides user login``` successfully for the first time it will print the location of where ```.fides_credentials``` is written to on your computer.
...
```console filename="Input As Options"
