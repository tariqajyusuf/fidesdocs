...
## Initialization
Running the fides ```init``` command will initialize a working directory that contains a Fides configuration ```toml``` file . After that initializiation, you will be able to perform other commands, like ```fides push``` and ```fides evalute```.
