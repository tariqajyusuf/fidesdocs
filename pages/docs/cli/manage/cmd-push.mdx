...
* <span class="nx-absolute -nx-mt-20" id="dry"></span>[```--dry```](#dry) - Only validate the syntax of system and dataset YAML but prevent persistance of any changes on the Server.
* <span class="nx-absolute -nx-mt-20" id="diff"></span>[```-diff```](#diff) - Ouput any changes between server and local resources as part of the command output response.
...
----------
```
## Example: Output changes between the server and local resources
