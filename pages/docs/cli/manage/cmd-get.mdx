...
The ```fides get``` command retrieves the specfified resources from the Fides Server and outputs the repsonse in YAML.
...
- administrating_department: 'Data Engineering'
  ...
  - data_categories:
    ...
    ingress:
    - data_warehouse
    name: 'Track customer details for ecommerce transactions as part of advertising.'
  system_dependencies:
  - ecommerce_app
  - data_warehouse
