...
This example provides a guided flow similar in behavior to the default command. However, here all data category labels provided will be stricly validated against the Fides taxonomy currently configured in your Fides instance.
...
In the example below, a nonexisting data category ```nonexistent_category``` has been provided for the ```customer.email``` field. The system responds with an error that the proposed category is invalid and will ask for a valid category again:
