...
The ```fides evaluate``` performs an automated privacy check on changes to resources to ensure they comply with Fides policies.
...
The ```fides evaluate``` can be used with Git hooks to continuously integrate privacy checks into an existing CI pipeline. For details on configuring automated privacy checks in a CI pipeline read the [Guide to Fides CI Checks](#LINK) here. 
...
For details on how policies are used read the [Guide to Fides Policies](#LINK) here.
...
This example executes an evaluation of all resources and all available policies. In this example, the resources are tested against two policies; a ```cpra_policy``` (California's Privacy Law) and a ```gdpr_policy``` (European Privacy Lawn). 
...
You can retrieve the results of this evaluation with either ```fides get [fides_key]``` wher the key is the identifier of the specific evaluation or with ```fides ls [evaluate]``` where evaluate is the list of resources to return.
