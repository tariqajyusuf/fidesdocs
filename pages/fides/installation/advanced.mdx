...
<Callout emoji="ⓘ">Want to quickly experiment with Fides? Clone the [source repository](https://github.com/ethyca/fides) to utilize the built-in Docker Compose file, check out the lightweight [start-up guide](./basic_setup), or use the [demo application](./sample_project) to go from "Zero to DSR Automation" in minutes!</Callout>
