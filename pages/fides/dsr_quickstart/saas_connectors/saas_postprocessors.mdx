...
Post-processors represent a form of data transformation. Fides can perform various processing methods to transform data received from a given endpoint into a format required for privacy requests.

## Configuration

Post-processors are configured within the `endpoints` section of your [SaaS configuration](./saas_config).
...
```
 ...
 <Callout> Order matters to the SaaS configuration. In the above example, **unwrap** will run first, and the output of unwrap will be used in the **filter strategy**.</Callout>
