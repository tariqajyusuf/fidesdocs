...
## Introduction
In this tutorial, you'll add and configure a privacy request integration for a system on the Data Map. By the end of this tutorial, you'll understand the relationship between systems and integrations as well as how to connect, configure and test an integration.
...
Typically these are API integrations to allow you to simplify and automate each request. For systems that do not have APIs and only accept privacy requests by email, Fides has an integration to automate email notifications to the vendor. Read about [automated email notifcations](#LINK) here.
...
* <span class="nx-absolute -nx-mt-20" id="integration-name"></span>[Integration Name](#integration-name) - The name of the integration being configured.
* <span class="nx-absolute -nx-mt-20" id="integration-description"></span>[Integration Description](#integration-description) - A description of the integration being configured.
* <span class="nx-absolute -nx-mt-20" id="connection-identifier"></span>[Connection Identifier](#connection-identifier) - A unique key to identify the integration within Fides.
...
In addition to these common fields, each integration will require credential information specific to that system's authentication requirements.
...
When you have provided the integration credentials, **Save** your integration settings.

### Test the Integration

Once your integration credentials are saved you can verify the credentials are working by clicking **Test connection**.
If the integration is working correctly you will be presented with a `Test suceeded` message like the one shown below:
...
This example used the Hubspot integration, for a complete list of integrations and instructions on generating credentials, visit the [integration registry](#LINK).