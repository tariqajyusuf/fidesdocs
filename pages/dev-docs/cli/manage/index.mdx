...
Fides' annotation workflow relies on six commands: ```generate```, ```annotate```, ```evaluate```, ```push```, ```pull``` and ```delete```. All of these commands require an [initialized](/docs/dev-docs/cli/init) working directory, and all of them act only upon the currently selected working directory. Although the Fides CLI can be used standalone, it is most powerful when used with the [Fides Server](/docs/dev-docs/cli/webserver).
...
For details see [the ```fides generate``` command.](/docs/dev-docs/cli/manage/cmd-generate)
...
For details see [the ```fides annotate``` command.](/docs/dev-docs/cli/manage/cmd-annotate)
...
For details see [the ```fides evaluate``` command.](/docs/dev-docs/cli/check/cmd-evaluate)
...
For details see [the ```fides push``` command.](/docs/dev-docs/cli/manage/cmd-push)
...
For details see [the ```fides pull``` command.](/docs/dev-docs/cli/manage/cmd-pull)

## Deleting
The ```fides delete``` command will delete a remote resource completely from the Fides Server.

For details see [the ```fides delete``` command.](/docs/dev-docs/cli/manage/cmd-delete)
...
* [The ```fides get``` command](/docs/dev-docs/cli/manage/cmd-get) retrieves the specified resources from the Fides Server and displays them as YAML.

* [The ```fides ls``` command](/docs/dev-docs/cli/manage/cmd-ls) gets a list of all resources from the server and displays them as YAML.

* [The ```fides parse``` command](/docs/dev-docs/cli/manage/cmd-parse) validates the syntax of all Fides YAML documents in the working directory.
