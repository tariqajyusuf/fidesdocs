...
> **Hands-on:** Try the [Fides: Get Started](/dev-docs/get-started) tutorials.  For more in-depth details on the ```scan``` command, check out the [Annotation Coverage tutorial](/dev-docs/privacy-requests/datasets/annotate-datasets).
