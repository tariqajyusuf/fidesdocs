...
An webhook is an HTTPS callback that you've defined on an [execution policy](/docs/policies/request-policies) to call an external REST API endpoint either *before* or *after* a privacy request executes.
...
The process below will define an `https` [Connection](/docs/databases/database-connections) that contains the details to make a request to your API endpoint, and then create a `PolicyPreWebhook` or a `PolicyPostWebhook`for a specific execution policy using that Connection.
