...
An webhook is an HTTPS callback that you've defined on an [execution policy](/dev-docs/configuration/policies/request-policies) to call an external REST API endpoint either *before* or *after* a privacy request executes.
...
The process below will define an `https` [Connection](/dev-docs/configuration/integrations/database-integrations/datasets) that contains the details to make a request to your API endpoint, and then create a `PolicyPreWebhook` or a `PolicyPostWebhook`for a specific execution policy using that Connection.
...
expire when your Redis cache expires represented by `default_ttl_seconds` in your Fides configuration. Read how to set [Fides configuration variables here](/dev-docs/configuration/configuration).
...
The `reply-to-token` expires after a set amount of time, the `privacy_request_delay_timeout` in your Fides configuration. Read how to set [Fides configuration variables here](/dev-docs/configuration/configuration).
