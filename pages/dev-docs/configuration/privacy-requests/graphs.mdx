...
For the first step, Fides uses the concept of an `identity`. In the Fides [Dataset](/user-guides/datasets) specification, any field may be marked with an `identity` notation:
