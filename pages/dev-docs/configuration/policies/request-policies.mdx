...
A DSR request policy (separate from a [CI Policy](/docs/dev-docs/cli/check), used for automated policy checks in CI) is a set of instructions, or Rules, that are executed when a user submits a [request](/docs/dev-docs/configuration/privacy-requests) to retrieve or delete their data. It describes how to access, mask, or erase data that matches specific data categories in submitted privacy requests.
...
| `drp_action` | *Optional.* A [Data Rights Protocol](/docs/dev-docs/configuration/privacy-requests/data-rights-protocol) action to associate to this policy. Accepted values are `access` (must be used with an [access Rule](#add-a-rule)) or `deletion` (must be used with an [erasure Rule](#add-an-erasure-rule)). |
...
| `action_type.erasure` | A data subject erasure request (or Right to be Forgotten). Matching data will be erased or [masked](/docs/dev-docs/configuration/privacy-requests/masking-strategies).
| `storage_destination` | Where Fides will upload the returned data for an `access` action. See [storage](/docs/dev-docs/configuration/privacy-requests/storage-destinations). |
| `masking_strategy` | How to erase data that applies to this `Rule`. See [Configuring Masking Strategies](/docs/dev-docs/configuration/privacy-requests/masking-strategies) |

<Callout>The `storage_key` must identify an existing storage object. Read about [configuring storage](/docs/dev-docs/configuration/privacy-requests/storage-destinations) here.</Callout>
