...
A [SaaS connector](saas-connectors) is defined in two parts: the [Dataset](/user-guides/datasets), and the SaaS configuration. The Dataset describes the data that is available from the connector, and the SaaS config describes how to connect to, and retrieve or update the data in the connector.
...
| `param_values.references` | These are the same as `references` in a [Dataset](/user-guides/datasets). It is used to define the source of the value for the given `param_value`.
