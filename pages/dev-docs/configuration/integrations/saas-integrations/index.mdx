...
For full examples of supported Integrations, see the [example configurations](/docs/dev-docs/configuration/integrations/saas-integrations/example-configs/adobe).
...
### Create a [Connection](/docs/dev-docs/configuration/integrations/database-integrations/datasets) of type `saas`
...
1. A SaaS Integration dataset cannot have any `identities` or `references` in the `fidesops_meta`. These relationships must be defined in the [SaaS config](/docs/dev-docs/configuration/integrations/saas-integrations/saas-config).
...
The example below creates a [mailchimp](/docs/dev-docs/configuration/integrations/saas-integrations/example-configs/mailchimp) saas connector, and would need the relevant mailchimp `secrets`.

Your `instance_key` will become the identifier for the related [Dataset](/docs/user-guides/integrations/database-integrations/datasets). By default, the saas connection config is enabled with write access.
