...
For full examples of supported Integrations, see the [example configurations](saas-integrations/example-configs/adobe).
...
1. A SaaS Integration dataset cannot have any `identities` or `references` in the `fidesops_meta`. These relationships must be defined in the [SaaS config](saas-integrations/saas-config).
...
The example below creates a [mailchimp]((saas-integrations/example-configs/mailchimp) saas connector, and would need the relevant mailchimp `secrets`.
