...
An [API connector](/dev-docs/configuration/integrations/api-integrations/api-config) is defined in two parts: the [Dataset](/user-guides/integrations/database-integrations/datasets), and the connection configuration. The Dataset describes the data that is available from the connector, and the connection config describes how to connect to, and retrieve or update the data in the connector.
...
## An example API config

This guide will use a API configuration to connect to [Mailchimp](/dev-docs/configuration/integrations/api-integrations/example-configs/mailchimp). The configuration schema defines:
...
The following is an example API config for Mailchimp:
...
|----|----|
| `fides_key` | Used to uniquely identify the connector, and to link a API config to a dataset. |
| `name` | A human-readable name for the connector. |
| `type` | Type of API connector. Choose from `hubspot`, `mailchimp`, or the other [available connectors](/dev-docs/configuration/integrations/api-integrations/example-configs/adobe), or use `custom` for other types. |
| `description` | Used to add a useful description. |
| `version` | Used to track different versions of the API config. |
...
#### External references
The `external_references` field is used to describe a list of external dataset references that a user must configure as part of the setup. These dataset references point to fields in a registered Fides dataset that are used to provide necessary values to the API integration execution. The location of these fields depends on the use-case, so they cannot be determined beforehand, and therefore must be configured at the time of setting up the API connector.
...
Fides also supports [OAuth2 authentication](/dev-docs/configuration/integrations/api-integrations/api-oauth2).
...
| `postprocessors` | An optional list of response post-processing strategies. We will ignore this for the example scenarios below but an in depth-explanation can be found under [API Post-Processors](/dev-docs/configuration/integrations/api-integrations/api-config).
| `pagination` | An optional strategy used to get the next set of results from APIs with resources spanning multiple pages. Details can be found under [API Pagination](/dev-docs/configuration/integrations/api-integrations/api-pagination).
...
Fides uses the available Datasets to [generate a graph](/dev-docs/configuration/privacy-requests/graphs#graphs-and-traversals) of all reachable data and the dependencies between Datasets. For API connectors, all the references and identities are stored in the `param_values`, and must merge both the connection config and Dataset to provide a complete picture for the graph traversal. Using Mailchimp as an example, the Dataset collection and API config endpoints for `messages` looks like this:
...
An example of the augmented Dataset with the API Config references would look like this:
