...
API integrations support two OAuth2 flows:
...
The placeholders are sourced from the values defined in the [`connector_params`](/dev-docs/configuration/integrations/api-integrations/api-config#integration-params) of your API config.
...
- Fides must be able to connect to the API or SaaS provider (Outreach, Salesforce, etc.).
- A **Client ID** and **Client Secret** must be generated target system or SaaS provider’s admin console.
  - This is dependent on your systems design or the SaaS vendor you're connecting to. Refer to the vendor or your system documentation.
- The connector using OAuth2 is configured using the steps for [how to configure a API connector](/dev-docs/configuration/integrations/api-integrations/api-config).
...
- The **Redirect URI** must be registered within the target API systems admin console.
...
![OAuth2 Workflow](/assets/img/saas_connections/oauth2_workflow.png "OAuth2 Workflow")
