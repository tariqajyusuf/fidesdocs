# Connect using API integrations

## What is an API integration?


A Fides API (Software as a Service) Integration allows a user to connect to a third-party SaaS application (e.g., Mailchimp, Stripe, Hubspot), and execute data access and erasure requests against that application. Fides represents your SaaS connections as a [Dataset](/user-guides/integrations/database-integrations/datasets), accessed via a [Connection](/dev-docs/configuration/integrations/database-integrations), and configured by a [SaaS config](/dev-docs/configuration/integrations/api-integrations/api-config) or any owned services applications you may have running.

## Supported API integrations

The current implementation of the Integration framework can support any application or third-party SaaS application that uses these features:
...
For full examples of supported Integrations, see the [example configurations](/dev-docs/configuration/integrations/api-integrations/example-configs/adobe).

## Configure an API Integration to a SaaS vendor
...
1. A SaaS Integration dataset cannot have any `identities` or `references` in the `fidesops_meta`. These relationships must be defined in the [SaaS config](/dev-docs/configuration/integrations/api-integrations/api-config).
...
The example below creates a [mailchimp](/dev-docs/configuration/integrations/api-integrations/example-configs/mailchimp) saas connector, and would need the relevant mailchimp `secrets`.
