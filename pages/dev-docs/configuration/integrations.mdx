...
A _Connection_ links your owned databases and [third-party applications](/dev-docs/configuration/integrations/api-integrations/api-config) to Fides, allowing Fides to execute privacy requests against your collections and fields.
...
The `system_status` filter surfaces either `database` or [`saas`-type](/dev-docs/configuration/integrations/api-integrations/api-config) connectors:
