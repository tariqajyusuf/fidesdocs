...
* A valid `access_token` created from a client credentials with adequate _**scopes**_ to create additional clients. Read how to [generate an access token](/docs/dev-docs/configuration/oauth/generating-tokens) here.
...
You can then create an access token for the client credentials by calling `POST /api/v1/oauth/token` with the new credentials and following the steps to [generate a token here](/docs/dev-docs/configuration/oauth/generating-tokens).
