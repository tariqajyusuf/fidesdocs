...
<Callout emoji="ⓘ">Want to quickly experiment with Fides? Clone the [source repository](https://github.com/ethyca/fides) to utilize the built-in Docker Compose file, alternatively check out the quick start guide [start-up guide](/dev-docs/get-started) to run a privacy request in minutes, or use follow the [basic installation](/dev-docs/get-started/installation).</Callout>
