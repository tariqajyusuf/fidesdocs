...
Storage destinations are associated to privacy request policies in their [rules](/dev-docs/configuration/policies/request-policies), allowing multiple storage destinations to be configured per privacy request policy.
...
On success, the response from the above endpoint will include a `storage_key` for each destination, which can be used when defining privacy request policy [rules](/dev-docs/configuration/policies/request-policies).
